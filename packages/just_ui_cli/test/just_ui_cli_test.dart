...
''');
  ...
  group('CLI Commands Tests', () {
    test('InitCommand generates config if pubspec exists', () async {
      ...
      expect(logs.any((l) => l.contains('Error: No pubspec.yaml found')), isTrue);
      ...
      await runCli(['init'], fs);
      expect(logs.any((l) => l.contains('success: JustUI initialized successfully')), isTrue);
...
''');
    ...
    test('AddCommand downloads files and resolves dependencies recursively', () async {
...
''');
      ...
      fs.file('mock_registry/index.json').writeAsStringSync(jsonEncode({
        'version': '1',
        'components': [
          {
            'name': 'button',
            ...
            'files': [
              {
                ...
                'checksum': 'sha256:${sha256.convert(utf8.encode('button_code')).toString()}'
              }
            ]
          },
          {
            ...
            'files': [
              {
                ...
                'checksum': 'sha256:${sha256.convert(utf8.encode('spacing_code')).toString()}'
              }
            ]
          }
        ]
      ...
      expect(fs.file('lib/ui/button/just_button.dart').readAsStringSync(), equals('button_code'));

      // Verify registry dependency is copied to tokensDir
      expect(fs.file('lib/tokens/spacing.dart').existsSync(), isTrue);
      expect(fs.file('lib/tokens/spacing.dart').readAsStringSync(), equals('spacing_code'));

      // Verify pub dependency is added
      expect(fs.file('pubspec.yaml').readAsStringSync(), contains('flutter_animate: "^1.0.0"'));
    });

    test('AddCommand handles circular dependencies gracefully using visited set', () async {
      fs.file('pubspec.yaml').writeAsStringSync('name: test');
      fs.file('justui.config.yaml').writeAsStringSync('''
components_dir: lib/ui
tokens_dir: lib/tokens
...
''');
            ...
            'files': [
              {
                ...
                'checksum': 'sha256:${sha256.convert(utf8.encode('a')).toString()}'
              }
            ]
          },
          {
            ...
            'files': [
              {
                ...
                'checksum': 'sha256:${sha256.convert(utf8.encode('b')).toString()}'
              }
            ]
          }
        ]
    ...
    test('DiffCommand detects changes and prints line-by-line diff on verbose mode', () async {
      fs.file('pubspec.yaml').writeAsStringSync('name: test');
      fs.file('justui.config.yaml').writeAsStringSync('''
components_dir: lib/ui
tokens_dir: lib/tokens
...
''');
      ...
      final componentHash = sha256.convert(utf8.encode('original')).toString();
      ...
      expect(logs.any((l) => l.contains('success: just_button.dart: Up to date')), isTrue);

      // Case 2: Modified checksum (non-verbose)
      fs.file('lib/ui/button/just_button.dart').writeAsStringSync('modified');
      logs.clear();
      await runCli(['diff', 'button'], fs);
      expect(logs.any((l) => l.contains('warning: just_button.dart: Modified locally')), isTrue);
      expect(logs.any((l) => l.contains('Line-by-line diff')), isFalse);

      // Case 3: Modified checksum (verbose)
      logs.clear();
      await runCli(['diff', 'button', '-v'], fs);
      expect(logs.any((l) => l.contains('Line-by-line diff for just_button.dart')), isTrue);
