...
mod tests {
    ...
    fn test_parse_tag_from_location() {
        assert_eq!(
            parse_tag_from_location("https://github.com/infinitedim/justui/releases/tag/v0.12.0"),
    ...
    fn test_plan_upgrade_matrix() {
        ...
        let action = plan_upgrade("0.12.0", &up_to_date_release, false, false).unwrap();
        assert_eq!(action, UpgradeAction::AlreadyUpToDate);

        // 1b. Up-to-date version with force=true
        let action_force = plan_upgrade("0.12.0", &up_to_date_release, false, true).unwrap();
        assert_eq!(
            action_force,
            UpgradeAction::NoMatchingAsset {
                target_version: "0.12.0".to_string()
            }
        );
        ...
        let action_check = plan_upgrade("0.12.0", &new_release, true, false).unwrap();
        assert_eq!(
            action_check,
        ...
        let action_dl = plan_upgrade("0.12.0", &new_release, false, false).unwrap();
        ...
        let action_no_asset = plan_upgrade("0.12.0", &no_asset_release, false, false).unwrap();
        assert_eq!(
            action_no_asset,
            UpgradeAction::NoMatchingAsset {
                target_version: "9.9.9".to_string()
        ...
        let action_checksum = plan_upgrade("0.12.0", &release_with_checksum, false, false).unwrap();
        ...
        let action_invalid = plan_upgrade("0.12.0", &invalid_semver_release, false, false).unwrap();
        assert_eq!(
            action_invalid,
