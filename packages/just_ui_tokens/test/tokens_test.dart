...
void main() {
  ...
  group('Dynamic HSL Color & Contrast Correction Validation', () {
    ...
    test('adjustLightnessForContrast adjusts color to meet target ratio', () {
      ...
      expect(corrected.contrastRatioWith(background), greaterThanOrEqualTo(4.5));
  ...
  group('Adaptive Motion Engine Validation', () {
    test('JustDuration.scaleForDistance scales and clamps durations correctly', () {
      ...
      expect(
        JustDuration.scaleForDistance(0, min: JustDuration.fast, max: JustDuration.slow),
        equals(JustDuration.fast),
      );
      expect(
        JustDuration.scaleForDistance(-50.0, min: JustDuration.fast, max: JustDuration.slow),
        equals(JustDuration.fast),
      );

      // Test mid-range distance: 150px / 1.5 pixels/ms = 100ms. If fast is 150ms, it should clamp to 150ms.
      expect(
        JustDuration.scaleForDistance(150.0, min: JustDuration.fast, max: JustDuration.slow),
        equals(JustDuration.fast),
      );

      // Test longer distance: 600px / 1.5 pixels/ms = 400ms. If slow is 400ms, it should return 400ms.
      expect(
        JustDuration.scaleForDistance(600.0, min: JustDuration.fast, max: JustDuration.slow),
        equals(JustDuration.slow),
      );

      // Test extremely long distance: 1500px / 1.5 pixels/ms = 1000ms. Clamps to max (slow - 400ms).
      expect(
        JustDuration.scaleForDistance(1500.0, min: JustDuration.fast, max: JustDuration.slow),
        equals(JustDuration.slow),
      );
    });

    testWidgets('JustMotionProfile.resolve respects system disableAnimations setting', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        MediaQuery(
  ...
  group('Advanced Primitives Validation', () {
    test('JustDynamicSurfaces.generateDarkSurface maintains seed hue and clamps saturation/lightness', () {
      ...
      final darkBg = JustDynamicSurfaces.generateDarkSurface(seed, lightness: 0.03);
      final darkCard = JustDynamicSurfaces.generateDarkSurface(seed, lightness: 0.07);
